...
user = '<username>' # set username values
schema = '<schema>' # schema values here
...
class db():
	...
	def __init__(self, user=user, schema=schema):
		self.schema = schema
		self.host = 'postgres://%s@127.0.0.1/%s' % (user, schema)
	...
	def parseShape(self):
		...
		for cmd in [{'db' : 'hotspot', 'parse' : 'Global_24h.shp', }, {'db' : 'country', 'parse': 'TM_WORLD_BORDERS-0.3.shp'}]:
			run = '/usr/local/bin/shp2pgsql -a -D -s 4326  -W latin1 -i %s/%s %s.%s | /usr/local/bin/psql -d %s' % (path, cmd['parse'], self.schema, cmd['db'], self.schema)
